...
class Client implements ClientInterface
{
    ...
    public function __call($method, $args)
    {
        ...
        return \mb_substr($method, -5) === 'Async'
            ? $this->requestAsync(\mb_substr($method, 0, -5), $uri, $opts)
    ...
    private function configureDefaults(array $config): void
    {
        ...
        if (!isset($this->config['headers'])) {
            ...
            foreach (\array_keys($this->config['headers']) as $name) {
                if (\mb_strtolower($name) === 'user-agent') {
                    return;
                }
            }
            $this->config['headers']['User-Agent'] = default_user_agent();
    ...
    private function applyOptions(RequestInterface $request, array &$options)
    {
        ...
        if (isset($options['json'])) {
            $options['body'] = \GuzzleHttp6\json_encode($options['json']);
            unset($options['json']);
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = Psr7\_caseless_remove(['Content-Type'], $options['_conditional']);
        ...
        if (!empty($options['auth']) && \is_array($options['auth'])) {
            $value = $options['auth'];
            $type = isset($value[2]) ? \mb_strtolower($value[2]) : 'basic';
            switch ($type) {
                case 'basic':
                    // Ensure that we don't have the header in different case and set the new value.
